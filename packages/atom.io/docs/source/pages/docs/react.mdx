...
`useI`—named in the sense "use input"—is a React hook that returns a stable callback to set (see [setState](/docs/atom-io#get-and-set-an-atom-ts)) an atom's value or a pass a new value to a writable selector's `set:` callback.

{/* DOCS REVIEW: This sentence has a typo ("or a pass") and the writable-selector case may need one concrete phrase about direct values vs updater callbacks. */}
...
{/* DOCS REVIEW: The bare and fallback forms have different return shapes. Should this explicitly say the fallback form removes the `"LOADING"` sentinel and always returns an object with `value`/`loading`/`error`? */}
