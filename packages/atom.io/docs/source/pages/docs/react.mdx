...
Call the returned `use()` hook without an argument when the provider is required. It
throws a descriptive error when the component is not below that key context's provider.

Pass a fallback key to `use(fallback)` when rendering outside the provider is valid. The
hook returns that key and logs a warning through the current atom.io store so the weaker
context guarantee remains visible during development.
