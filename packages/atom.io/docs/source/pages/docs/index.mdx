...
For fetched, queried, and RPC-backed state that needs clearer cache identity or more
deliberate normalization, see the [remote data guide](/docs/remote-data). It covers
compact query atoms, loadable families keyed by input, and local-index patterns for
filtered or optimistic list screens.
