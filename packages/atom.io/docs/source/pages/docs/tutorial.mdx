...
{/* DOCS REVIEW: This code sample is useful, but the indentation inside the `CodeBlock` string is uneven enough that the nesting is hard to follow. Should the snippet be formatted or shortened with explicit omissions? */}
...
{/* DOCS REVIEW: This reset snippet uses `getState`/`resetState` inside the transaction body instead of the transaction toolkit shown elsewhere. Is that intentional, or could it teach the wrong transaction pattern? */}
