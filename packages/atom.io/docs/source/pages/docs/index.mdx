...
<table-wrapper>
  <table>
    <tbody>
      ...
      <tr>
        <td>
          <code>timelineFamily</code>
        </td>
        <td>
          Partition atom families into independently addressable histories.
        </td>
      </tr>
      <tr>
        <td>
...
For long-lived editors, declare a retention policy to bound undo history. Capacity
is measured in complete undo checkpoints, so selector writes and transactions are
never split. When the limit is exceeded, the oldest complete checkpoint is removed.

<Exhibit src="core/timeline/retain-bounded-history.ts" />

Recording after an undo removes the redo branch before applying the limit.
`clearTimeline` removes all retained history without changing the policy, while
disposing an ordinary timeline removes both. Every timeline-family member applies
the family's policy independently, including members recreated after disposal.
...
<Exhibit src="core/timeline/create-a-timeline-family.ts" />
...
<Exhibit src="core/timeline/use-a-timeline-family-in-a-silo.ts" />
...
<Exhibit src="core/timeline/undo-a-transaction-across-timelines.ts" />
...
<Exhibit src="react/use-timeline-family.tsx" label="React" />

<Exhibit src="solid/use-timeline-family.tsx" label="Solid" />
