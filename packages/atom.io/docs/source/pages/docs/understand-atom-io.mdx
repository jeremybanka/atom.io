...
import { UnderstandAtomSandbox } from "c/UnderstandAtomSandbox";
...
<UnderstandAtomSandbox concept="laziness" client:visible />

<Exhibit src="guides/understand-atom-io/laziness.ts" />

`doubledSelector` is never computed until something reads it with `getState`, `useO`, or
subscribes to it. A subscription runs the selector once so atom.io can discover the
selector's current dependencies. If its cached value is later evicted, atom.io only
recomputes it when someone is still subscribed or a new read asks for it.
...
<Exhibit src="guides/understand-atom-io/counter.test.ts" />
...
<Exhibit src="guides/understand-atom-io/browser-path-sync.ts" />

The browser version can lean on `window.location` to keep a piece of state [in sync with the current pathname](/docs/typesafe-router).

<Exhibit src="guides/understand-atom-io/node-config-sync.ts" />
...
<Exhibit src="guides/understand-atom-io/transactions-and-timelines.ts" />
...
<Exhibit src="guides/understand-atom-io/search-box.tsx" />
