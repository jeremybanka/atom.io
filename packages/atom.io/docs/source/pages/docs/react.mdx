...
If no fallback is passed to `createKeyContext`, `use()` returns `Key | undefined` and
still warns outside the provider. The hook never throws when its declared return type can
be satisfied.
