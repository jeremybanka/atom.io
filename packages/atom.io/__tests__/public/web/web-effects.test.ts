...
describe(`storageSync`, () => {
	it(`should sync a value to localStorage`, () => {
		...
		expect(localStorage.getItem(`myString`)).toBe(JSON.stringify(`B`))
		$ = new Silo({
			name: `web-effects-store-${i}`,
			lifespan: `ephemeral`,
			isProduction: false,
		...
		expect(localStorage.getItem(`myString`)).toBeNull()
	})
...
describe(`searchParamSync`, () => {
	it(`should sync a value to the URL query params`, () => {
		...
		expect(getSearchParams().get(`myString`)).toBe(JSON.stringify(`D`))
		expect(getSearchParams().get(`keep`)).toBe(JSON.stringify(`C`))
		...
		$.setState(myStringAtom, null)
		expect(getSearchParams().has(`myString`)).toBe(false)
		expect(getSearchParams().get(`keep`)).toBe(JSON.stringify(`C`))
	})
