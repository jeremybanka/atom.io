import AppAnchor from "x/guides/typesafe-router/app-anchor.gen";
import BunStaticRoutes from "x/guides/typesafe-router/bun-static-routes.gen";
import CurrentRoute from "x/guides/typesafe-router/current-route.gen";
import NginxStaticHosting from "x/guides/typesafe-router/nginx-static-hosting.gen";
import PathnameState from "x/guides/typesafe-router/pathname-state.gen";
import RouteSelection from "x/guides/typesafe-router/route-selection.gen";
import RouteShape from "x/guides/typesafe-router/route-shape.gen";
...
Define route structure once as shared, typed data with TreeTrunks. TreeTrunks route trees use object keys for path segments, `null` for terminal leaves, `optional(...)` for optional branches, and `$paramName` segment names for dynamic segments. From that tree, derive these types:

- `Route`: a tuple-like path representation, such as `["grants", string, "applications"]`.
- `Pathname`: a slash-prefixed string representation, such as `/grants/abc/applications`.
- `PathnameWithSearch`: the route pathname plus a query string, such as `/grants/abc/applications?tab=budget`.
...
<RouteShape client:load />

Dynamic segment markers, such as `$grantId`, belong in the TreeTrunks route tree; the derived `Route` type exposes the actual segment value. Keep TreeTrunks as the default mechanism for this pattern so runtime validation and TypeScript route types come from the same source.
...
- intercept same-origin anchor clicks whose `href` starts with `/`;
...
Bun's dynamic segment syntax may differ from the route-tree syntax. For example, a route-tree segment like `$grantId` can be translated to Bun's `:grantId` segment before adding it to the static routes object.

<BunStaticRoutes client:load />
