...
affected loadable state makes the remote system the source of truth again. This
matters most for data that only the remote can assign or normalize, such as
database ids, timestamps, counts, permission-derived fields, or canonical row
shapes. For more patterns around fetched and RPC-backed state, see the
