...
`useO`—named in the sense "use output"—is a React hook that returns the current value of an atoms or selectors in a React components (and dispatches a re-render whenever there is a new value).
