...
Use it when membership matters, but order does not. The runtime API is set-like:
`add`, `delete`, `clear`, `has`, and iteration work the way they do on `Set`.
The "list" part is about its transceiver shape: `UList` can serialize to a JSON array
and report compact add/delete/clear updates for transactions and timelines.

<Exhibit src="transceivers/u-list.ts" />
...
<Exhibit src="transceivers/o-list.ts" />
