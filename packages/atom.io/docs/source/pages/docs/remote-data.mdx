import RemoteDataClient from "x/guides/remote-data/client.gen";
...
import ResetAfterMutation from "x/guides/remote-data/reset-after-mutation.gen";
import RemoteDataServer from "x/guides/remote-data/server.gen";
...
- After a mutation succeeds, deliberately update optimistic local state, then
  reset affected loadable row atoms, key indexes, or query family members when
  the remote owns part of the truth, such as ids, timestamps, counts, or
  normalized row shapes.

### reset after mutations

Use reset as the resync step after mutations that change remote-owned state.
Optimistic writes can keep the interface responsive, but reset asks the remote
for the canonical result.

<ResetAfterMutation client:load />
...
<RemoteDataServer client:load soft />

The client derives a typed client directly from that router:

<RemoteDataClient client:load soft />
