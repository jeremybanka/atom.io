...
<table-wrapper>
  <table>
    <tbody>
      ...
      <tr>
        <td>
          <code>createKeyContext</code>
        </td>
        <td>
          Create a named context for supplying an application key to a React subtree.
        </td>
      </tr>
      <tr>
        <td>
...
`createKeyContext` lets a React subtree supply a domain key once while keeping family
resolution explicit. This is useful when many descendants address state with the same
document, form, tenant, or other local identity and passing that key through every
component would add noise.

<Exhibit src="react/create-key-context.tsx" />

Pass a fallback key when creating the context. When `use()` runs outside the provider,
it returns that fallback and warns once for each misplaced rendered consumer. The
warning identifies the consumer's render branch and includes both its component trace
and React owner path when available. StrictMode replay and ordinary rerenders of the
same branch do not repeat the warning, while distinct consumers and remounts are each
reported. Warnings also occur during server rendering and are not limited to
development; whether they are displayed depends on the current store's logger
configuration.

If no fallback is passed to `createKeyContext`, `use()` returns `Key | undefined`
without warning outside the provider. Choosing that return type explicitly accepts a
missing context so the caller can handle it. The hook never throws when its declared
return type can be satisfied.

The key context does not resolve family members itself. Components still pass the
resulting key to `useO`, `useI`, `useTL`, or other keyed APIs. This confines React context
to choosing a key while atom.io's state dependencies remain explicit and usable outside
React.
